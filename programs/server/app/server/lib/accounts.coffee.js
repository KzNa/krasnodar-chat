...
Accounts.validateLoginAttempt(function(login) {                        // 2
  var ref, ref1, validEmail;                                           // 98
  login = RocketChat.callbacks.run('beforeValidateLogin', login);      // 98
  if (login.allowed !== true) {                                        // 100
    return login.allowed;                                              // 101
  }                                                                    //
  ...
  if (indexOf.call((ref1 = login.user) != null ? ref1.roles : void 0, 'admin') < 0 && login.type === 'password' && RocketChat.settings.get('Accounts_EmailVerification') === true) {
    validEmail = login.user.emails.filter(function(email) {            // 109
      return email.verified === true;                                  // 110
    });                                                                //
    if (validEmail.length === 0) {                                     // 112
      throw new Meteor.Error('no-valid-email');                        // 113
      return false;                                                    // 114
    }                                                                  //
  }                                                                    //
  RocketChat.models.Users.updateLastLoginById(login.user._id);         // 98
  Meteor.defer(function() {                                            // 98
